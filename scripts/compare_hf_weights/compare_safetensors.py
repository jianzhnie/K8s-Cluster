...
def _compare_safetensors_pair(file1_path,
    ...
    if not only_diff:
        ...
        if missing_in_1 or missing_in_2:
            output_lines.append(f"  Missing keys: {len(missing_in_1) + len(missing_in_2)}\n")
...
def _compare_dir_pair(source_dir,
    ...
    print(f"Scanning files in {source_dir} and {target_dir} with pattern='{pattern}' recursive={recursive}...")
    ...
    source_map, source_collisions = _build_file_map(source_files, source_dir, match_mode)
    target_map, target_collisions = _build_file_map(target_files, target_dir, match_mode)

    if source_collisions or target_collisions:
        print("Warning: duplicate match keys detected; results may be incomplete.")
        if source_collisions:
            print(f"  Source duplicate keys (showing up to 10): {source_collisions[:10]}")
        if target_collisions:
            print(f"  Target duplicate keys (showing up to 10): {target_collisions[:10]}")
    ...
    if pairs:
        ...
        with ProcessPoolExecutor(max_workers=jobs) as ex:
            fut_to_name = {
                ex.submit(_compare_safetensors_pair,
                          src,
                          tgt,
                          tolerance,
                          verbose,
            ...
            for fut in _tqdm_iter(as_completed(fut_to_name), total=len(pairs), desc="Comparing"):
...
if __name__ == "__main__":
    ...
    group.add_argument("--source-dir", help="Directory containing source safetensors files")
    parser.add_argument("--target", help="Path to the target safetensors file")
    parser.add_argument("--target-dir", help="Directory containing target safetensors files")
    ...
    parser.add_argument("--only-diff", action="store_true", help="Only print mismatches (suppress summary and OK logs)")
    parser.add_argument("--pattern", default="*.safetensors", help="Glob pattern for directory mode (default: *.safetensors)")
    ...
    code, out = _compare_safetensors_pair(args.source,
                                          args.target,
                                          args.tolerance,
                                          args.verbose,
                                          args.only_diff,
